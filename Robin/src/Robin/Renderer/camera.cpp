...
namespace Robin
{
	orthorgraphic_camera::orthorgraphic_camera(float left, float right, float bottom, float top)
		: m_projection_matrix(glm::ortho(left, right, bottom, top, -1.f, 1.f)), m_view_matrix(1.f), m_position(0.f)

	{
		m_view_projection_matrix = m_projection_matrix * m_view_matrix;
	}
	orthorgraphic_camera::~orthorgraphic_camera()
	{

	}

	void orthorgraphic_camera::set_projection(float left, float right, float bottom, float top)
	{
		m_projection_matrix = glm::ortho(left, right, bottom, top, -1.f, 1.f);
		m_view_projection_matrix = m_projection_matrix * m_view_matrix;
	}
	void orthorgraphic_camera::create_view_matrix()
	{
	...
	void perspective_camera::set_projection(float fov, float width, float height, float near_plane, float far_plane)
	{
		m_projection_matrix = glm::perspective(glm::radians(fov), width / height, near_plane, far_plane);
		create_view_matrix();
	}
